...
- 硬件遮挡查询
  - 思想：和z缓冲器中的内容进行比较时，用户可以查询硬件来找到一组多边形是否可见，且这些多边形通常是复杂物体的包围体，如果没有多边形可见，就将其裁剪掉。
  - 硬件实现对查询的多边形进行光栅化，并将其和z缓冲器去比较

- 层次z缓冲HZB
  - 八叉树维护场景模型，对场景遮挡区域层次剔除
  - 将画面的z缓冲器作为图形金字塔，对单个基元和边界体积进行层次z缓冲

<img src="《Real_time Rendering 3rd》读书笔记.assets/182810FE888D183208946A7326079770.png" alt="img" style="zoom: 50%;" />

- 层次遮挡映射HOM
  - 分层图像空间剔除
  - 思想：每帧建立一个分层深度缓冲区，用于遮挡测试。并且在每个级别使用不透明度阈值来确定是否有足够的要渲染的对象是可见的。如果只有一小部分可见，那么该对象被剔除
- 遮挡地平线算法OH
  - 基于点的可见性算法
  - 定位地平线的位置，在此之后的模型不被渲染
- 遮挡物收缩与视锥扩张算法
  - 基于单元的可见性方法
  - 遮挡物收缩可以使用基于点的遮挡算法生成基于单元的可见性

## 层次细节（LOD）

- 思想：当物体对渲染图的贡献越小，就是用越简单的形式表达该物体
- 游戏广泛应用的优化技术
- 步骤：
  - 生成：生成不同细节的模型表示
  - 选择：基于某种准则选取一个层次细节模型
    - 基于距离的LOD选取：在不同距离段选取不同模型<img src="《Real_time Rendering 3rd》读书笔记.assets/76D402B268789F51015B9D2745B14AD1.png" alt="img" style="zoom:50%;" />
    - 基于投影面积的LOD选取：投影面积越大细节越丰富
    - 基于滞后的LOD选取：基于距离的LOD，当r增大时，使用上一行LOD，当r减小时，使用下一行LOD距离，避免同一距离模型快速变大变小<img src="《Real_time Rendering 3rd》读书笔记.assets/CAFE65DD1E686C2C4F1FC2B1DD1CD6A4.png" alt="img" style="zoom:50%;" />
    - 基于距离和投影面积的LOD选取（最常用）
    - 还可以基于物体的重要程度，运动，焦点，整体可见性作为LOD选取方案
  - 切换：从一个细节模型切换到另一个细节模型
    - 忽然切换模型会引起玩家的注意（突越）
    - 切换算法
      - 离散几何LOD：不同的表示是不同图元数量的同一模型，突越现象严重
      - 混合LOD：切换时线性混合，得到一种平滑的切换。但代价高
      - 透明LOD（alphaLOD）：并没有使用同一物体的很多不同细节实例，每个物体只有一个实例，随着距离增大，alpha减小，超过一定距离物体就完全消失。比离散几何LOD连续一些，可以避免突越现象，完全消失的物体不需要渲染，加速效果好
      - 连续LOD&几何形变LOD：**连续层次细节**是根据距离修模型的多边形数量。**几何形变层次细节**是基于简化生成的一组离散模型

<img src="《Real_time Rendering 3rd》读书笔记.assets/86DB47825E11D0A71A220AB1BA52135F.png" alt="img" style="zoom:50%;" />
...
- 渲染系统有一个固定的渲染帧率（硬实时，时间临界）
- 给这类系统一个特定的时间段，必须在该时间段完成任务，时间到必须停止处理
- 对场景的可见物体，自适应选取细节层次，从而满足固有帧率的要求
- 这个算法在场景由预测性，因为可见性物体的LOD选取基于预期帧率和可见物体

### 大型模型的渲染

- 使用一个四叉树数据结构u覆盖地球表面，每个叶子节点根据具体内容使用不同数据结构。
- 为保持帧率，即将进入视野的模型区域，在需要之前从磁盘分页
- 可以用裁剪图管理大型纹理

### 点渲染

- 用点集表示物体表面来渲染。随后用高斯滤波填充渲染点之间的间隙

# 渲染管线瓶颈和优化

- 图形渲染管线

![img](《Real_time Rendering 3rd》读书笔记.assets/40DCD39CB55207EBFCD6C14BDB4864D7.png)

- 瓶颈：三个主要阶段（应用程序阶段，几何阶段，光栅化阶段）的通信
- 优化管线的思路：先将渲染速度最大化，再让非瓶颈部分和平静部分小号相同多的时间（在等待时间完成更多任务），但现在新的架构会自动加载平衡计算资源

## 渲染管线的瓶颈定位策略

- profiler查找调用耗时
- 控制变量法：每个测试减少特定阶段执行的工作量。如果其中一个测试的导致每秒帧数增加，就找到了瓶颈

<img src="《Real_time Rendering 3rd》读书笔记.assets/0FBD6EC139130338D08ACC1D72B78AD0.png" alt="img" style="zoom:50%;" />

### 的瓶颈定位

| 测试阶段     | 测试操作       | 测试方法                                                     |
| ------------ | -------------- | ------------------------------------------------------------ |
| 光栅化阶段   | 光栅化操作     | 将颜色输出的位深度从32(24)位减小到16位。                     |
|              | 像素着色器     | 减小屏幕分辨率。                                             |
|              | 帧缓冲带宽     | 改变颜色缓冲的位深度，或者深度缓冲的位深度                   |
|              | 纹理带宽       | 使用大量正等级mipmap细节层次的偏差，让纹理获取访问非常粗糙的mipmap金字塔级别，来有效减少纹理尺寸。 |
|              | 片元着色器     | 先判断不是帧缓冲器，再调整屏幕分辨率                         |
| 几何阶段     | 顶点与索引传输 | 调整顶点格式大小                                             |
|              | 顶点变换       | 对于可编程的顶点变换，改变顶点程序的长度；对于固定功能的顶点变换，可以修改镜面光照或者纹理坐标生成的状态修改负载 |
| 应用程序阶段 |                | 用profiler工具查看CPU的占用情况                              |
|              |                | 发送一些其他阶段工作量极小或者根本不工作的数据               |
|              |                | 对CPU降频                                                    |
|              |                | 排除法：如果GPU没有瓶颈，那CPU就是瓶颈所在                   |
...
| ------------ | ------------------------------------------------------------ |
| CPU          | 减小资源锁定：                                               |
|              | 批次的尺寸最大化：若使用了三角形带，则使用**退化三角形**将不相交的条带拼接起来，这样一次可以发送多条三角形带，从而共享材质；使用**纹理页**，在单个draw call中大宋使用了多个纹理的几何体；使用**shader分支**增加单个批次大小从而合批；将顶点着色器常量内存作为**矩阵查找表**使用；尽可能远地往管线下端推迟决策。 |
| 应用程序阶段 | 为编译器打开优化标志                                         |
| 内存层面     | 内存池：管理内存池，减少增加删除操作                         |
| 代码层面     | 善用单指令多数据流SIMD                                       |
|              | 少用除法                                                     |
|              | 用泰勒展开代sin，cos等                                       |
|              | 常用小函数内联                                               |
|              | 使用低精度数据                                               |
| API调用      | 使用某种形式的实例，如森林场景，与其位每一棵树调用API，不如单次调用渲染树的许多副本（享元模式） |
| 几何阶段     | 减少顶点传输的开销：顶点格式中使用尽可能少的位。产生可推导的位，而非全部存储。以相对连续的方式访问顶点数据 |
|              | 顶点处理的优化：减少要处理的顶点数，使用顶点处理LOD          |
|              | 将顶点数据存储再压缩格式里                                   |
| 光照计算     | 判断那些物体需要光照                                         |
|              | 将静态光的漫反射光和环境光提前存储再顶点颜色中               |
|              | 控制光源数量，或禁用光源                                     |
|              | 延迟着色                                                     |
| 光栅化阶段   | 背面裁剪                                                     |
|              | 纹理压缩                                                     |
